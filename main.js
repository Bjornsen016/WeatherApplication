...
import { dayOfWeekAsString, capitalizeFirstLetter, generate24HourTime } from "./utils.js";
import { citys, backgrounds, getCitysFromLocalStorage, saveCitysToLocalStorage } from "./consts.js";
...
function renderNow(data) {
  ...
  const FastAverageColor = window.FastAverageColor;
  const Colors = new FastAverageColor();
  const info = backgrounds.filter((item) => item.name === data.weather[0].main)[0];
  const container = document.getElementById("container").style;

  Colors.getColorAsync(info.url).then((color) => {
    container.backgroundColor = color.hex;
    color.isDark ? (container.color = "white") : (container.color = "black");
  });
  ...
  document.getElementById("window").style.backgroundImage = `url('${info.url}')`;
}
...
function renderToday(item) {
  ...
  main.innerHTML += `
    <article>
        ...
        <img src="http://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png" alt="${capitalizeFirstLetter(
